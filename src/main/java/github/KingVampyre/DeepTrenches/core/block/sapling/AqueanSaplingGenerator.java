...
public class AqueanSaplingGenerator implements FeaturesSaplingGenerator {
    ...
    private static final ImmutableMap<BlockRotation, Identifier> BIG_AQUEANS = ImmutableMap.<BlockRotation, Identifier>builder()
    ...
    private static final ImmutableMap<BlockRotation, Identifier> MEDIUM_AQUEANS = ImmutableMap.<BlockRotation, Identifier>builder()
    ...
    private static final ImmutableMap<BlockRotation, Identifier> SMALL_AQUEANS = ImmutableMap.<BlockRotation, Identifier>builder()
    ...
    public ConfiguredFeature<?, ?> createTreeFeature(Random random, boolean sapling) {
        ...
        if (random.nextInt(7) == 0) {
            Identifier id = BIG_AQUEANS.get(rotation);

            return CONFIGURED_FEATURE.get(id);
        }

        if (random.nextInt(3) == 0) {
            Identifier id = MEDIUM_AQUEANS.get(rotation);

            return CONFIGURED_FEATURE.get(id);
        }

        Identifier id = SMALL_AQUEANS.get(rotation);
